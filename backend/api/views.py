...
class RoomsRemove(generics.DestroyAPIView):
    @swagger_auto_schema(
        request_body=openapi.Schema(
            type=openapi.TYPE_OBJECT,
            properties={
...
class GetRoomsAsync(generics.ListAPIView):
    """
    API view to retrieve a list of all rooms.
    """
    permission_classes = [AllowAny]
    queryset = api_models.Rooms.objects.all()
    serializer_class = api_serializer.RoomSerializer

    @swagger_auto_schema(
        operation_description="Retrieve a list of all rooms.",
        responses={200: api_serializer.RoomSerializer(many=True)}
    )
